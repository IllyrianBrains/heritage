...
```mermaid
flowchart LR
    A[Kërkesa e dërguar] --> B{Përgjigje brenda afatit?}
    B -- Po --> C[Regjistro rezultatin]
    B -- Jo --> D[Kujtesë me shkrim]
    D --> E{Përgjigje?}
    E -- Po --> C
    E -- Jo --> F[Ankesë te Komisioneri / Agjencia]
    F --> G[Gjykata ose organi më i lartë]
```
